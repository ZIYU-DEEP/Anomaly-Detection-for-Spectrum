...
anomaly_folder = str(sys.argv[5])  # e.g. ryerson_ab_train_sigOver
shift_eval = int(sys.argv[6])
batch_size = int(sys.argv[7])
gpu_no = int(sys.argv[8])
normal_folder_control = normal_folder + '_control'
anomaly_folder_control = anomaly_folder + '_control'
...
valid_true = np.reshape(utils.windowed_true_control(full_x_valid, shift_eval, predict_size), (-1, predict_size, 36))
...
anom_true_list = [utils.windowed_true_control(i, shift_eval, predict_size)
                  for i in abnormal_series_list]
...
for i in range(len(anom_hat_list)):
    ...
    anom_true = np.reshape(anom_true_list[i], (-1, predict_size, 36))
